...
| **K3013** | `<what> requires a type annotation` / `requires an explicit return type`                 | A position the checker does not infer: an agent parameter, a `use` binder, or an agent in a **recursive** group — recursion has no fixed point to infer from, so `-> T` and `with …` are both required there.    | Write it. For a long ceiling row, name it once as an effect-row synonym — `type bot_ceiling = ai.observation \| discord.credential \| io` — and use the name. Where annotations are conventional rather than demanded: [the boundary is spelled, the inside is inferred]({docs}/{currentVersion}/concepts/language-reference#annotations-the-boundary-is-spelled-the-inside-is-inferred). |
...
| **K3016** | `Could not infer the type argument(s) [T] from this application; supply them explicitly` | A type argument nothing in the application constrains. The live case is a `use handler` for a generic request whose parameters do not name the generic — `request mk[a]() -> a` handled as `request mk() { … }`. | Supply it: `request mk[integer]() { … }`. Inference reads the arguments only, so an annotation on the binding does not help. Where writing a type argument is the convention rather than a repair: [the boundary is spelled, the inside is inferred]({docs}/{currentVersion}/concepts/language-reference#annotations-the-boundary-is-spelled-the-inside-is-inferred).                     |
...
| ``The actual type is `unknown`, so nothing is known about the value``                                | Usually a field or key that the type does not declare — check the name. Otherwise narrow the value with a `match` before using it. This is also how an unconstrained type argument surfaces: inference reads the arguments, so a `T` reachable only through the result or a callback's return type solves to `unknown` — write it, as in `json.validate[order](value = body)`. |
